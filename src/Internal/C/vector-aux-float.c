...
/* int toScalarR(int code, KFVEC(x), FVEC(r)) { */
/*     REQUIRES(rn==1,BAD_SIZE); */
/*     DEBUGMSG("toScalarR"); */
/*     float res; */
/*     integer one = 1; */
...
/* float scnrm2_(integer*, const complex*, integer*); */
/* float scasum_(integer*, const complex*, integer*); */

/* int toScalarC(int code, KQVEC(x), FVEC(r)) { */
/*     REQUIRES(rn==1,BAD_SIZE); */
/*     DEBUGMSG("toScalarC"); */
/*     float res; */
/*     integer one = 1; */
...
int mapR(int code, KFVEC(x), FVEC(r)) {
    int k;
...
inline float abs_complex(complex z) {
    return sqrt(z.r*z.r + z.i*z.i);
}

inline complex complex_abs_complex(complex z) {
    complex r;
    r.r = abs_complex(z);
    r.i = 0;
    return r;
...
inline complex complex_signum_complex(complex z) {
    complex r;
    float mag;
    if (z.r == 0 && z.i == 0) {
        r.r = 0;
...
/* int mapC(int code, KQVEC(x), QVEC(r)) { */
/*     TCF* x2p = (TCF*)xp; */
/*     TCF* r2p = (TCF*)rp; */
/*     int k; */
/*     REQUIRES(xn == rn,BAD_SIZE); */
...
inline complex complex_f_math_fun(complex (*cf)(complex), complex a)
{
  complex c;
  complex r;
...
int mapValR(int code, float* pval, KFVEC(x), FVEC(r)) {
    int k;
    float val = *pval;
    REQUIRES(xn == rn,BAD_SIZE);
    DEBUGMSG("mapValR");
...
int mapValC(int code, complex* pval, KQVEC(x), QVEC(r)) {
    TCF* x2p = (TCF*)xp;
    TCF* r2p = (TCF*)rp;
    int k;
    TCF val = * (TCF*)pval;
...
int zipR(int code, KFVEC(a), KFVEC(b), FVEC(r)) {
...
int zipC(int code, KQVEC(a), KQVEC(b), QVEC(r)) {
    REQUIRES(an == bn && an == rn, BAD_SIZE);
    TCF* a2p = (TCF*)ap;
    TCF* b2p = (TCF*)bp;
    TCF* r2p = (TCF*)rp;
...
int vectorScan(char * file, int* n, float**pp){
    FILE * fp;
    fp = fopen (file, "r");
    if(!fp) {
        ERROR(BAD_FILE);
    ...
    int nbuf = 100*100;
    float * p = (float*)malloc(nbuf*sizeof(float));
    int k=0;
    float d;
    int ok;
    for (;;) {
        ...
        if (k==nbuf) {
            nbuf = nbuf * 2;
            p = (float*)realloc(p,nbuf*sizeof(float));
            // printf("R\n");
        }
        p[k++] = d;
    }
...
inline float urandom() {
    ...
    */
    ...
    return (float)nrand48(state) / (float)max_random;
}
...
float gaussrand(int *phase, float *pV1, float *pV2, float *pS)
{
	float V1=*pV1, V2=*pV2, S=*pS;
	float X;
...
int random_vector(unsigned int seed, int code, FVEC(r)) {
    int phase = 0;
    float V1,V2,S;
...
inline float urandom(struct random_data * buffer) {
    int32_t res;
    random_r(buffer,&res);
    return (float)res/RAND_MAX;
}


// http://c-faq.com/lib/gaussian.html
float gaussrand(struct random_data *buffer,
                 int *phase, float *pV1, float *pV2, float *pS)
{
	float V1=*pV1, V2=*pV2, S=*pS;
	float X;
	...
	if(*phase == 0) {
		do {
      float U1 = urandom(buffer);
			float U2 = urandom(buffer);
...
/* int sort_valuesD(KFVEC(v),FVEC(r)) { */
...
int round_vector(KFVEC(v),FVEC(r)) {
    int k;
    for(k=0; k<vn; k++) {
        rp[k] = round(vp[k]);
    }
...
int round_vector_i(KFVEC(v),IVEC(r)) {
    int k;
    for(k=0; k<vn; k++) {
        rp[k] = round(vp[k]);
    }
...
int round_vector_l(KFVEC(v),LVEC(r)) {
    int k;
    for(k=0; k<vn; k++) {
        rp[k] = round(vp[k]);
    }
...
/* int constantR(float * pval, FVEC(r)) { */
/*     DEBUGMSG("constantR") */
/*     int k; */
/*     float val = *pval; */
/*     for(k=0;k<rn;k++) { */
...
/* int constantC(complex* pval, QVEC(r)) { */
/*     DEBUGMSG("constantC") */
/*     int k; */
/*     complex val = *pval; */
/*     for(k=0;k<rn;k++) { */
...
int float2long(KFVEC(x),LVEC(y)) CONVERT_IMP
...
int long2float(KLVEC(x),FVEC(y)) CONVERT_IMP
...
int compareD(KFVEC(x),KFVEC(y),IVEC(r)) {
    COMPARE_IMP
}
...
/* int chooseD(KIVEC(cond),KFVEC(lt),KFVEC(eq),KFVEC(gt),FVEC(r)) { */
/*     CHOOSE_IMP */
/* } */
...
/* int chooseC(KIVEC(cond),KQVEC(lt),KQVEC(eq),KQVEC(gt),QVEC(r)) { */
/*     CHOOSE_IMP */
/* } */
...
int reorderD(IVEC(k), KIVEC(strides),KIVEC(dims),KFVEC(v),FVEC(r)) {
    REORDER_IMP
}
...
int reorderC(IVEC(k), KIVEC(strides),KIVEC(dims),KQVEC(v),QVEC(r)) {
    REORDER_IMP
}
