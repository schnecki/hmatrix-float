...
import           Internal.Algorithms (Field, Herm, linearSolveSVD, mTm)
...
>>> linspace 5 (-3,7::Float)
[-3.0,-0.5,2.0,4.5,7.0]
it :: Vector Float

>>> linspace 5 (8,3:+2) :: Vector (Complex Float)
[8.0 :+ 0.0,6.75 :+ 0.5,5.5 :+ 1.0,4.25 :+ 1.5,3.0 :+ 2.0]
it :: Vector (Complex Float)
...
class Build d f c e | d -> c, c -> d, f -> e, f -> d, f -> c, c e -> f, d e -> f
  where
    -- |
    -- >>> build 5 (**2) :: Vector Float
    -- [0.0,1.0,4.0,9.0,16.0]
    -- it :: Vector Float
    --
    -- Hilbert matrix of order N:
    --
    -- >>> let hilb n = build (n,n) (\i j -> 1/(i+j+1)) :: Matrix Float
...
it :: (Vector Float, Herm Float)
-}
meanCov :: Matrix Float -> (Vector Float, Herm Float)
