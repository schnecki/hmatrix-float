...
type AssocMatrix = [((Int,Int),Float)]

data CSR = CSR
        { csrVals  :: Vector Float
        , csrCols  :: Vector CInt
        , csrRows  :: Vector CInt
        , csrNRows :: Int
        , csrNCols :: Int
...
data GMatrix
    ...
    | Diag
        { diagVals :: Vector Float
        , nRows    :: Int
        , nCols    :: Int
        }
    | Dense
        { gmDense :: Matrix Float
        , nRows   :: Int
        , nCols   :: Int
        }
--    | Banded
...
type  V t = CInt -> Ptr Float -> t
type SMxV = V (IV (IV (V (V (IO CInt)))))

gmXv :: GMatrix -> Vector Float -> Vector Float
...
-}
infixr 8 !#>
(!#>) :: GMatrix -> Vector Float -> Vector Float
(!#>) = gmXv
...
toDense :: AssocMatrix -> Matrix Float
